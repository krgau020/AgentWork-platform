...
export default function PoliciesPage() {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold text-zinc-900">Policies</h1>
      ...
      {admin && (
        ...
        >
          <h2 className="text-sm font-semibold text-zinc-900 mb-4">Create new policy</h2>
      ...
      <div className="mt-6 space-y-3">
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-zinc-200 shadow-sm px-6 py-8 text-center text-sm text-zinc-500">
            No policies found
          </div>
        ) : (
          policies.map((policy) => {
                  ...
                  {isRenamingThis ? (
                    <div className="flex items-center gap-2 flex-1">
                      <input
                        ...
                        className="flex-1 px-2.5 py-1.5 rounded-lg border border-green-400 text-zinc-900 focus:outline-none focus:ring-2 focus:ring-green-500 text-sm font-semibold"
                      />
                      <button
                        onClick={() => handleRename(policy.policy_id)}
                        disabled={renaming || !editingPolicyName.trim()}
                        className="text-xs bg-green-500 hover:bg-green-600 disabled:bg-green-300 text-black font-medium rounded-lg px-3 py-1.5 transition-colors"
                      >
                        {renaming ? 'Saving...' : 'Save'}
                      </button>
                      <button
                        onClick={cancelRename}
                        className="text-xs text-zinc-500 hover:text-zinc-700 font-medium px-2 py-1.5"
                      >
                        Cancel
                      </button>
                      {renameError && (
                  ...
                  {admin && !isRenamingThis && (
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        ...
                        className="text-xs text-zinc-500 hover:text-zinc-700 font-medium border border-zinc-200 rounded-lg px-3 py-1.5 transition-colors"
                      >
                        Rename
                      </button>
                      <button
                        onClick={() => openAddStatement(policy)}
                        className="text-xs bg-green-500 hover:bg-green-600 text-black font-medium rounded-lg px-3 py-1.5 transition-colors"
                      >
                        Add statement
                      </button>
                    </div>
                ...
                {isExpanded && (
                  <div className="border-t border-zinc-200">
                    ...
                    {policy.statements.length === 0 ? (
                      <p className="px-6 py-4 text-sm text-zinc-400">No statements</p>
                    ) : (
                      <table className="w-full">
                        ...
                        <tbody className="divide-y divide-zinc-100">
                          {policy.statements.map((stmt) => {
                            ...
                            if (isEditingThis) {
                              return (
                                <tr key={stmt.statement_id} className="bg-green-50">
                                  <td className="px-4 py-3">
                                    <input
                                      ...
                                      className="w-full px-2 py-1.5 rounded border border-green-300 text-zinc-900 text-sm font-mono focus:outline-none focus:ring-1 focus:ring-green-500"
                                    />
                                  </td>
                                  <td className="px-4 py-3">
                                    <input
                                  ...
                                  <td className="px-4 py-3">
                                    <select
                                      ...
                                      className="px-2 py-1.5 rounded border border-green-300 text-zinc-900 text-sm focus:outline-none focus:ring-1 focus:ring-green-500"
                                    >
                                  ...
                                  <td className="px-4 py-3">
                                    <div className="flex items-center gap-2">
                                      <button
                                        ...
                                        disabled={stmtSaving}
                                        className="text-xs bg-green-500 hover:bg-green-600 disabled:bg-green-300 text-black font-medium rounded px-2.5 py-1 transition-colors"
                                      >
                            ...
                            return (
                              <tr key={stmt.statement_id} className="hover:bg-zinc-50">
      ...
      >
        <form onSubmit={handleAddStatement} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1.5">Resource</label>
            <input
              type="text"
              required
              value={stmtResource}
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1.5">Action</label>
            <input
              type="text"
              required
              value={stmtAction}
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1.5">Effect</label>
            <select
              value={stmtEffect}
