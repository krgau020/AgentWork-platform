...
export default function SolutionsPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-zinc-900">Solutions</h1>
      ...
      {!loading && !error && solutions.length === 0 && (
        <div className="bg-zinc-50 border border-zinc-200 rounded-xl px-6 py-10 text-center">
          <p className="text-zinc-500 font-medium">No solutions registered yet.</p>
          <p className="text-zinc-400 text-sm mt-1">
            An admin can register a solution via <code className="text-xs bg-zinc-100 px-1.5 py-0.5 rounded">POST /api/v1/registry/register</code>.
          </p>
        </div>
      )}
...
function SolutionCard({ solution: s }: { solution: ServiceResponse }) {
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm p-5 flex flex-col gap-4">
      {/* Header */}
      ...
      {/* Info */}
      <div>
        <p className="text-base font-semibold text-zinc-800">{s.display_name || s.name}</p>
      ...
      <div className="space-y-1.5 text-xs text-zinc-500">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-zinc-700">{accessLabel}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-zinc-400">Health</span>
          <span className="font-mono text-zinc-600">{s.health_endpoint}</span>
        </div>
        {s.registered_at && (
          <div className="flex items-center gap-2">
